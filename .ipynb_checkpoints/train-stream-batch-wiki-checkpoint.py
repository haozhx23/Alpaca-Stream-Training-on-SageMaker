...
def train():
    ...
    if int(os.environ['MAX_STEPS']) > 100:
        training_args.max_steps = int(os.environ['MAX_STEPS'])
    ...
    train_data = train_data.shuffle(buffer_size=100000).map(
        partial(
            # tokenize_prompt_gen,
            tokenize_wiki_batch,
            tokenizer
    ...
    ############################
    print('---1-DDD---: ', timeit.default_timer())
    trainer = Trainer(model=model, tokenizer=tokenizer, args=training_args, **data_module)
    trainer.train()
    print('---2-DDD---: ', timeit.default_timer())
    trainer.save_state()
