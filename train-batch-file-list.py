...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer):
        ...
        # !touch '/tmp/tempfiles/tmp_file_0.jsonl'
        # for i, fstr in enumerate(flist):
        #     # print(i, fstr)
        #     i1 = i+1
        #     src_file_name = f'/tmp/tempfiles/tmp_file_{i}.jsonl'
        #     out_file_name = f'/tmp/tempfiles/tmp_file_{i1}.jsonl'
        #     !cat {fstr} {src_file_name} > {out_file_name}
...
def train():
    ...
    if 0 == LOCAL_RANK:
        ...
        print(f'------rank {LOCAL_RANK} finished model cp-------')
        
        print('-------DD-------DATACPY1: ', time.time())
        os.system("./s5cmd sync {0} {1}".format(os.environ['DATA_S3_PATH'], data_args.data_path))
        print('-------DD-------DATACPY2: ', time.time())
        print(f'------rank {LOCAL_RANK} finished data cp-------')
    ...
    print('-------DD-------Train1: ', time.time())
    trainer = Trainer(model=model, tokenizer=tokenizer, args=training_args, **data_module)
    trainer.train()
    print('-------DD-------Train2: ', time.time())
    trainer.save_state()
