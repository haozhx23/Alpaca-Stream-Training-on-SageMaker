...
def get_item_from_file(single_file_path):
    json_list_file = open(single_file_path, 'rb')
    # jlf = json_list_file.read()
    jlf = json.load(json_list_file)
    json_list_file.close()
    return jlf

def get_s3_files_list(s3_file_path):
    import boto3, urllib
    s3 = boto3.client('s3')

    def _parse_bucket_key(s3uri):
        output_url = urllib.parse.urlparse(s3uri)
        bucket = output_url.netloc
        key = output_url.path[1:]
        return bucket, key

    bukname, pathky = _parse_bucket_key(s3_file_path)

    pathlist = s3.list_objects_v2(Bucket=bukname, Prefix=pathky)
    fileslist = list(set([i['Key'] if not i['Key'].endswith('/') else '#' for i in pathlist['Contents']]))
    fileslist.remove('#')
    # print('-S3fileslist-', fileslist)
    return fileslist
############################
